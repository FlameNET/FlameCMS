<div id="blog-articles" class="blog-articles" itemscope="itemscope" itemtype="http://schema.org/Blog">
	...
	$news_query = $con->query("SELECT * FROM news WHERE id <= '".$news_first."' ORDER BY `id` DESC LIMIT 8");
	$counter = 1;
	...
	<div class="article-wrapper">
		<a href="<?php echo $cms_add.'blog/'.$news['id'].'/'.CleanString($news['title']); ?>/" itemprop="url">
		<div class="article-image" style="background-image:url(assets/images/news/<?php echo $news['image']; ?>.jpg)">
			...
			<div class="article-content" itemprop="blogPost" itemscope="itemscope" itemtype="http://schema.org/BlogPosting">
				...
				<meta itemprop="thumbnailUrl" content="<?php echo $cms_add ?>assets/images/news/<?php echo $news['image']; ?>.jpg" />
			</div>
		</a>
		<div class="article-meta">
			<span class="publish-date" title="<?php echo $news['date']; ?>"><?php echo ago(strtotime($news['date'])); ?></span>
			<a href="<?php echo $cms_add.'blog/'.$news['id'].'/'.CleanString($news['title']); ?>/#comments" class="comments-link"><?php echo $news['comments']; ?></a>
		</div>
		<span class="clear"><!-- --></span>
	</div>
	<?php
